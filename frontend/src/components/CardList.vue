  ...
  <div v-show="!responseOK" class="my-3 text-black">
    <Form @submit="Submit()">
      ...
      <div class="justify-center">
        <button
          class="text-white font-semibold border border-vue-green rounded-sm shadow-lg w-24 p-2 justify-center bg-vue-green inline-flex items-center ease-in hover:scale-110 hover:bg-gradient-to-r from-emerald-600 duration-300"
          type="submit"
          :disabled="disabled"
        >
          <svg
            v-show="disabled"
            class="inline mr-2 w-8 h-8 text-white animate-spin fill-gray-font"
            viewBox="0 0 100 101"
            fill="none"
    ...
    <p v-show="rejected" class="text-rose-500 font-weight-bold mb-0 mt-2">
      {{ t("rejected") }}
    </p>
  </div>
  <ThankYou :response-ok="responseOK" :date="data.date" :time="data.time" />
</template>
